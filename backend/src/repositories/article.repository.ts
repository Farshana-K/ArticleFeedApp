...
import {
  ...
  IArticleRepository,
  UpdateArticleInput,
  InteractionCounterField,
} from "../contracts/article.repository.interface";
...
export class ArticleRepository implements IArticleRepository {
  ...
  findByIdAndAuthor(
    ...
    return ArticleModel.findOne({ _id: id, author: authorId })
      .populate("category", "name slug")
      .populate("author", "firstName lastName");
  }
  updateByAuthor(
  ...
  async incrementInteractionCounters(
    articleId: string,
    changes: Partial<Record<InteractionCounterField, number>>,
  ): Promise<void> {
    await ArticleModel.updateOne({ _id: articleId }, { $inc: changes });
  }
}
